...
#    analysis_name <- sprintf("8taxa_1000chars_GTR_%i",i)
    
    posterior_predictive_data <- read.table(paste0(dir,"simulated_data_",dataset_name,".csv"),header=TRUE,sep=",",check.names=FALSE)
    posterior_original_data   <- read.table(paste0(dir,"empirical_data_",dataset_name,".csv"),header=TRUE,sep=",",check.names=FALSE)
