...
\node[snode] (sigmay) at (5,2) {\large $\sigma_y$};
\node[constnode] (lambday) at (5,4) {\large $\lambda_y$};
\draw[taro] (sigmay) -- (y) ;
\draw[taro] (lambday) -- (sigmay) ;
...
\node[obnode] (x) at (0,0) {\large $x_i$};
\node[snode] (sigmax) at (-1,2) {\large $\sigma_x$};
\node[constnode] (lambdax) at (-1,4) {\large $\lambda_x$};
\draw[taro] (sigmax) -- (x) ;
\draw[taro] (lambdax) -- (sigmax) ;
\draw[taro] (muy) -- (y) ;
\draw[dtaro] (x) -- (muy) ;
\node[snode] (mux) at (-2,0) {\large $\mu_x$};
    \draw[taro] (mux) -- (x) ;
    \node[constnode] (mumux) at (-3,2) {\large $\mu_{\mu_x}$};
    \draw[taro] (mumux) -- (mux) ;
    \node[constnode] (sigmamux) at (-2,2) {\large $\sigma_{\mu_x}$};
    \draw[taro] (sigmamux) -- (mux) ;
