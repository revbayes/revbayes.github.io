...
{% citet Stadler2018 %} show how to compute the density of the "sampled tree", which is obtained by pruning all unsampled lineages after asymmetric species identities have been assigned in the complete tree ({% ref fig_sampled %}). This gives rise to the "Fossilized Birth Death Speciation Process" (implemented as `FBDSP` in RevBayes). This is the model we will be using in this tutorial. It is important to note that the tips in the sampled tree represent the age of the youngest sample for each species.
...
{% assign mcmc_script = "mcmc_CEFBDP.Rev" %}
{% assign fbdr_script = "model_FBDP.Rev" %}
...
complete, you will return to editing `mcmc_CEFBDP.Rev` and complete the
...
> Save your current working version of `mcmc_CEFBDP.Rev` in the `scripts` directory.
{:.instruction}

We will now move on to the next Rev file and will complete
`mcmc_CEFBDP.Rev` in section {% ref Exercise-CompleteMCMC %}.
...
we use the `FBDSP` distribution function
...
>You have completed the FBD model file. Save `model_FBDSP.Rev` in the `scripts` directory.
{:.instruction}
...
(`mcmc_CEFBDP.Rev`).
    ...
    source("scripts/mcmc_CEFBDP.Rev")
