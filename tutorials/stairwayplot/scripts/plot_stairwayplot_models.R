...
for (model in c("iid", "GMRF", "GMRF2", "HSMRF", "HMSRF2") ) {

  for (run in 1:4) {

    population_size_log = paste0("output/StairwayPlot_",model,"_ordered_Ne_run_",run,".log")
    interval_change_points_log = paste0("output/StairwayPlot_",model,"_ordered_times_run_",run,".log")
    ...
    p <- plotPopSizes(df) +
         ggtitle( paste0("Lampyris noctiluca (Munich) - ",model," - Rep ", run ) ) + theme(plot.title = element_text(hjust = 0.5, size=10, face="bold.italic")) +
    ...
    ggplot2::ggsave(filename=paste0("figures/StairwayPlot_",model,"_",run,".pdf"), plot=p, width=4, height=4)
    ggplot2::ggsave(filename=paste0("figures/StairwayPlot_",model,"_",run,".png"), plot=p, width=4, height=4)
  }
}
