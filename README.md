...
5. You can check if the modified website builds and looks correct by running
   `bundle exec jekyll serve` (see below on setting up jekyll).
6. Open a pull request (PR) for the branch on github at https://github.com/revbayes/revbayes.github.io
7. The PR will automatically run tests that the website builds and the tutorial tests pass.
   PRs requests for the website do not currently require review, so you can merge them yourself if checks pass.
8. When the PR is merged, the website will be updated automatically.
   It will probably take less than 10 minutes.
